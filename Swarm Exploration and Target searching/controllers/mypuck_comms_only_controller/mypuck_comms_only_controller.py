    ...
    for index,time in enumerate(timings):
        ...
        for item in object:
            ...
            if item.get_colors()== [1,0,0]:
                ...
                target_pos_img = item.get_position_on_image()
                ...
                x_pos_of_target = X_pos+target_pos[0]
                z_pos_of_target = Z_pos+target_pos[2]
                #Condition to check if robot is close enough to the target
                print("Target is at position","X =",x_pos_of_target,"Z =",z_pos_of_target)
                print("Relative_pos =",target_pos,"Model =",target_model)
                #print("Img_pos =",target_pos_img)
                #print("Up value =",up_value)
