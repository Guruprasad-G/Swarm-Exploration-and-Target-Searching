...
def graph_updation(current_node,r,u,l,d):
    ...
    global graph
    if u and (current_node-10 not in graph[current_node]):
        addEdge(graph,current_node,current_node-10)
    ...
    if r and (current_node+1 not in graph[current_node]):
        addEdge(graph,current_node,current_node+1)
    if d and (current_node+10 not in graph[current_node]):
        addEdge(graph,current_node,current_node+10)
...
def next_direction(current_node,next_node):
    ...
    if current_node+1 == next_node:
        return right
    elif current_node-10 == next_node:
        return up
